...
async def _execute_run(self, run: _RunLaunch, *, _api_server) -> None:
    ...
    def _thinking_cb(text: Optional[str]) -> None:
        if run_id not in self._run_streams:
            return
        with suppress(Exception):
            loop.call_soon_threadsafe(run.put_event, _run_event(run_id, "thinking.delta", text=text or ""))
    ...
    def _tool_start_cb(tool_call_id: str, function_name: str, function_args: Dict[str, Any]) -> None:
        ...
        with suppress(Exception):
            loop.call_soon_threadsafe(run.put_event, _run_event(
                run_id, "tool.started", tool_call_id=tool_call_id,
                tool=function_name, preview=label))

    def _tool_complete_cb(
        tool_call_id: str, function_name: str, function_args: Dict[str, Any], function_result: Any,
    ) -> None:
        ...
        with suppress(Exception):
            loop.call_soon_threadsafe(run.put_event, _run_event(
                run_id, "tool.completed", tool_call_id=tool_call_id, tool=function_name))
